...
/*
  changes from 1.0.0:
    - implemented way to check line if busy before writing packet
      raises flag if line was busy
    - dynamic pin declaration
    - minor naming conventions changed
*/
...
#define ow_SetPin_OUTPUT (*_pinPort_DDR |= _pinMask)
#define ow_SetPin_INPUT (*_pinPort_DDR &= _pinMaskNot)

#define ow_SetPin_HIGH (*_pinPort_PORT |= _pinMask)
#define ow_SetPin_LOW (*_pinPort_PORT &= _pinMaskNot)

#define ow_ReadPin (*_pinPort_PIN & _pinMask)
...
public:
  /*
    setPin();
      ...
      any current read or write operation will be immediately stopped unless pin is the same

      inputs: ( (uint8_t) Pin(input/output) )
  */
  void setPin(uint8_t);
  ...
  */
  ...
  /*
    writeFlag():
      return previous write result

      returns: (bool) previous write result
                  false = no error, packet write Success
                  true = write insert ERROR (line Busy)
  */
  inline bool writeFlag();
...
private:
  ...
  // keep track of current pin number:
  uint8_t _pin;

  // false if pin not set yet. Becomes true as soon as first pin is set ('setPin()')
  bool _init = false;
  ...
  */
  ...
  /*
    "_prevWriteState"
      previous write state/result
        false = no error, packet write Success / Init state
        true = write insert ERROR (line Busy)
  */
  bool _prevWriteState = false;
...
void OneWire::setPin(uint8_t pin)
{
  if (!_init || pin != _pin)
  {
    // stop all current reading and writing actions:
    _runState = 0;
    // reset CURRENT pin:
    ow_SetPin_INPUT; // set pin to input
    ow_SetPin_HIGH;  // set to pullup

    // set pin registers:
    pin %= 20; // make sure pin does not go out of bounds (0-19 only)
...
void OneWire::read(uint8_t *packet, uint8_t &bits, uint8_t maxBits, uint16_t maxParse_us = 0)
{
  ...
  if (_init) // only read if initialized
  {
    _packet = packet;
    _returnBits = &bits;
    _bits = maxBits;
...
void OneWire::write(uint8_t *packet, uint8_t bits)
{
  ...
  if (_init) // only write if initialized
  {
    _packet = packet;
    _bits = bits;
    _bitCount = 0;
...
inline void OneWire::run()
{
  ...
  case 2: // write
  {
    ...
    for consistency*/
    {
      _pinState ? ow_SetPin_HIGH : ow_SetPin_LOW;
    }
    switch (_sendStage)
    {
      // Data Bits
    ...
    case 0:
    {
      // run 8 times. Smallest HIGH pulse should always be 6-pulses
      if (_sendBitStage < 8)
        _sendBitStage++;

      // LOW value! Another device is using the line:
      if (!ow_ReadPin)
      {
        _prevWriteState = true; // flag write error
        _runState = 0;          // stop transmission
      }
      else if (_sendBitStage == 8)
      {
        _sendStage = 1;    // set to start bit
        _sendBitStage = 0; // reset bitStage
      }
    }
    break;
    default:
...
bool OneWire::writeFlag()
{
  return _prevWriteState;
}
...
#undef ow_SetPin_OUTPUT
#undef ow_SetPin_INPUT

#undef ow_SetPin_HIGH
#undef ow_SetPin_LOW
