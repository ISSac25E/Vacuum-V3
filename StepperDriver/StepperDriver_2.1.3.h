...
public:
  /*
    StepperDriver():
      ...
      the 'currentPosition' of the stepper will only be changed AFTER the odd step is completed
  */
  ...
  */
  ...
  /*
    returns true if current stepper is not moving and ready.
    only use 'setStepper' if 'ready' returns true

    returns: true = stepper not moving/ready
             false = stepper in motion/not ready
  */
  bool ready();

  /*
    setStepper():
      set new stepper variable pointer.
      you can read the stepper current position through the original variable
      the lib will take care of modifying the variable accordingly.
      Modifying the original variable may cause unexpected behavior.
      If a modification is needed, do so through 'step(int32_t)'

      inputs: ( (int32_t &)stepper position variable reference )
  */
...
private:
  ...
  */
  ...
  /*
    ...
    input: ( (bool)step polarity, (bool)direction: true = CW(clockwise), false = CCW(counter-clockwise) )
  */
  ...
  */
  int32_t _targetPos;
  /*
    initial current (absolute) position variable of stepper in steps
    ass soon as a new variable is assigned, '_initCurrentPos' is not needed anymore
  */
  int32_t _initCurrentPos;
};
...
bool StepperDriver::ready()
{
  return !_timerRunning;
}

void StepperDriver::setStepper(int32_t &stepperRef)
{
  // always disable timer first before modifying vars:
  TimerInterval.disableTimer();
  _timerRunning = false;

  _currentPos = &stepperRef;
...
void StepperDriver::step(int32_t newPos)
{
  // always disable timer first before modifying vars:
  TimerInterval.disableTimer();
  _timerRunning = false;
...
{
  __timerFunctClassPnt__->_stepCallback(__timerFunctClassPnt__->_stepPolarity, __timerFunctClassPnt__->_direction);

  if (!__timerFunctClassPnt__->_stepPolarity)
  {
  ...
  __timerFunctClassPnt__->_stepPolarity = !__timerFunctClassPnt__->_stepPolarity;
}
#endif
