...
TuyaWifi::TuyaWifi(void)
{
  ...
  /* nothing to do here */
#else
  wifi_work_state = WIFI_STATE_UNKNOWN;
#endif
}
...
void TuyaWifi::uart_service(void)
{
  ...
  // check if heartbeat timer exceeds:
  if (millis() - _heartbeatTimer > TY_MODULE_HB_TIMEOUT_H_ms)
  {
    _moduleUartError = true;
    wifi_work_state = MODULE_UART_ERROR;
  }
 ...
 * @return {*}
 */
void TuyaWifi::heart_beat_check(void)
{
  unsigned char length = 0;
  static unsigned char mcu_reset_state = TY_FALSE;

  // Check if heart beat appears too fast:
  if (millis() - _heartbeatTimer < TY_MODULE_HB_TIMEOUT_L_ms && mcu_reset_state)
  {
    _moduleUartError = true;
    wifi_work_state = MODULE_UART_ERROR;
  }
  else
  {
    if (_moduleUartError)
    {
      _moduleUartError = false;
      mcu_reset_state = TY_FALSE;
      wifi_work_state = WIFI_STATE_UNKNOWN;
    }
  }

  if (wifi_work_state == WIFI_STATE_UNKNOWN)
  {
    mcu_reset_state = TY_FALSE;
  }
